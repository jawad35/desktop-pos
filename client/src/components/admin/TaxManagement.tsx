...
import { Calendar, Download, FileText, Banknote, CheckCircle, AlertCircle, TrendingUp, TrendingDown } from "lucide-react";
...
export default function TaxManagement() {
  ...
  // Fetch tax summary
  const { data: taxSummary, refetch: refetchSummary } = useQuery({
    queryKey: ["taxSummary", dateRange],
    queryFn: async () => {
      const result = await api.getTaxSummary(dateRange.start, dateRange.end);
      return result || { total_tax_collected: 0, total_tax_paid: 0, balance_due: 0, total_sales: 0 };
    },
  });
  ...
  const getTaxPeriods = () => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const paidRecords = taxPayments.filter(p => 
        p.period_start === periodStart && p.period_end === periodEnd
      );
      const totalPaid = paidRecords.reduce((sum, p) => sum + p.amount, 0);
  ...
  const totalTaxCollected = taxSummary?.total_tax_collected || 0;
  const totalTaxPaid = taxSummary?.total_tax_paid || 0;
  const totalBalanceDue = taxSummary?.balance_due || 0;
            ...
            <table className="w-full">
              <thead className="bg-muted/50 border-b">
                <tr>
                  ...
                  <th className="text-right p-3">Sales (PKR)</th>
              ...
              <tbody>
                {periods.map((period) => {
                  const periodSales = sales.filter(sale => {
                    const saleDate = new Date(sale.created_at);
                    return saleDate >= new Date(period.periodStart) && saleDate <= new Date(period.periodEnd);
                  }).reduce((sum, sale) => sum + parseFloat(sale.total || 0), 0);
                  
                  return (
                    <tr key={period.periodStart} className="border-b hover:bg-muted/30">
                      <td className="p-3 font-medium">{period.periodName}</td>
                      <td className="p-3 text-right">{formatPKR(periodSales)}</td>
                      <td className="p-3 text-right font-semibold">{formatPKR(period.taxCollected)}</td>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-blue-800">How Tax Tracking Works:</p>
            <ul className="text-xs text-blue-700 mt-2 space-y-1 list-disc list-inside">
              <li>Every sale automatically records tax based on your tax rate setting</li>
              <li>Monthly periods show total tax collected from customers</li>
              <li>Record tax payments with government challan numbers for audit trail</li>
              <li>Export reports for FBR filing and audit purposes</li>
              <li>Keep all challan numbers for at least 6 years as required by law</li>
            </ul>
          </div>
        </div>
      </div>
