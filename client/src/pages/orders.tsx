...
export default function Orders() {
    ...
    const { data: productsData, isLoading: productsLoading } = useQuery<{
        ...
        queryKey: ["/api/paginate-products", filters, page],
        queryFn: async () => {
            ...
            const url = `/api/paginate-products?${queryParams.toString()}`;
    ...
    const { data: salesman = [] } = useQuery<Category[]>({
        queryKey: ["/api/employees/salesmen"],
    });
    ...
    const searchSaleMutation = useMutation({
        mutationFn: async (receiptNumber: string) => {
            const token = localStorage.getItem("token");
            const response = await fetch(`/api/sales/receipt/${receiptNumber}`, {
                headers: token ? { Authorization: `Bearer ${token}` } : {},
            });
        ...
        onSuccess: (data) => {
            ...
            // Add sale items to cart for return
    ...
    const addToCart = (product: any) => {
        ...
        if (existingItem) {
            ...
            setCart(cart.map(item =>
                item.id === product.id
                    ? {
                        ...
                        availableStock: product.stock // Update available stock
                    }
                    : item
            ));
        } else {
            setCart([...cart, {
                ...
                availableStock: product.stock // Add available stock
            }]);
        }
    };
    ...
    const handleProcessPayment = () => {
        ...
        const saleData = {
            ...
            employeeId: employeeId || null, // Add employee ID here
            items: cart.map(item => ({
                productId: item.id,
                quantity: item.quantity,
    ...
    const handleAddScannedProduct = async () => {
        ...
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`/api/products?barcode=${scannedCode}`, {
                headers: token ? { Authorization: `Bearer ${token}` } : {},
            });
    ...
    const handleProcessReturn = () => {
        ...
        //     description: "Please search for a valid receipt first",
        //     variant: "destructive",
        //   });
        //   return;
        // }
                                ...
                                products.map((product: any) => (
                                    ...
                                    >
                                        <CardContent className="p-4">
                                            ...
                                            <p className="text-primary font-semibold">{formatPKR(product.sellingPrice)}</p>
